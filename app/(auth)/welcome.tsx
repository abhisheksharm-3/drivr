...
const Welcome = () => {
  ...
  return (
    <SafeAreaView className="flex items-center justify-between h-full">
      ...
      <CustomButton title={isLastSlide ? "Get Started" : "Next"} onPress={() => isLastSlide ? router.replace("/(auth)/sign-up") : swiperRef.current?.scrollBy(1)} className="w-11/12 mt-10 mb-2" />
    </SafeAreaView>
  );
};
